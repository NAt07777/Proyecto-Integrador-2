...
//Función para generar un nuevo código en el método post.
async function generateCode(collection) {
    const docMaxCode = await collection.find().sort({codigo: -1}).limit(1).toArray();
    const maxCode = docMaxCode[0]?.codigo ?? 0;
